...
s <- 0.2	#prob individual learner succesfully acquires adaptive behavior
U <- 0.1  #prob environment changes states
mu_s <- 0.05	#mortality rate of social learners
...
n_IVAa[1] <- 20
n_IVAn[1] <- 20
n_An[1] <- n_VVAn[1] + n_IIAn[1] + n_OOAn[1] + n_OVAn[1] + n_IVAn[1]
n_Aa[1] <- n_VVAa[1] + n_IIAa[1] + n_OOAa[1] + n_OVAa[1] + n_IVAa[1]
...
u[1] <- 0
N[1] <- n_VVAa[1] + n_VVAn[1] + n_IIAa[1] + n_IIAn[1] + n_OOAa[1] + n_OOAn[1] + n_OVAa[1] + n_OVAn[1] + n_IVAa[1] + n_IVAn[1] + n_VVJH[1] + n_VVJS[1] + n_IIJH[1] + n_IIJS[1] + n_OOJH[1] + n_OOJS[1] + n_OVJH[1] + n_OVJS[1] + n_IVJH[1] + n_IVJS[1]
...
for (t in 1:timesteps)
{
  ...
  n_Aa[t+1] <- n_VVAa[t+1] + n_IIAa[t+1] + n_OOAa[t+1] + n_OVAa[t+1] + n_IVAa[t+1]
  n_An[t+1] <- n_VVAn[t+1] + n_IIAn[t+1] + n_OOAn[t+1] + n_OVAn[t+1] + n_IVAn[t+1]
  N[t+1] <- n_VVAa[t+1] + n_VVAn[t+1] + n_IIAa[t+1] + n_IIAn[t+1] + n_OOAa[t+1] + n_OOAn[t+1] + n_OVAa[t+1] + n_OVAn[t+1] + n_IVAa[t+1] + n_IVAn[t+1] + n_VVJH[t+1] + n_VVJS[t+1] + n_IIJH[t+1] + n_IIJS[t+1] + n_OOJH[t+1] + n_OOJS[t+1] + n_OVJH[t+1] + n_OVJS[t+1] + n_IVJH[t+1] + n_IVJS[t+1]
...
lines( 1:length(p_IV) , p_IV , col="orange" )

legend("topleft" , c("VV", "II", "OO" , "OV" , "IV") , lty=1 , lwd=c(1,1,1,1,1) , col=c("violet", "red", "black" , "green" , "orange") , pch=c(15,15,15,15,15) ,bty='n')
...
# lines( 1:length(q_OO) , q_OO , lty=2 , col="blue")
